...
class ModelNet40_Views(Dataset):
    """Multiple Projection Views of `clean` ModelNet40
    ...
    def __init__(self, cfg):
        ...
        self.dataset_dir = cfg.modelnet40_views_root
        ...
        for item in os.listdir(self.dataset_dir):
            d = os.path.join(self.dataset_dir, item)
            if os.path.isdir(d):
                self.pc_view_path.append(os.path.join(self.dataset_dir, str(pc_id)))
                pc_id += 1

        self.labels = []
        with open(os.path.join(self.dataset_dir, 'labels.txt')) as fin:
            lines = fin.readlines()
            for line in lines:
