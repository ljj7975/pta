...
class PointDA_ShapeNet(Dataset):
    def __init__(self, cfg):
        self.lm3d = cfg.lm3d
        self.template = text_prompts

        self.dataset_dir = os.path.join('data/xset/pointda', 'shapenet')
    ...
    def load_data(self, name2idx, split):
        ...
        for cls in os.listdir(self.dataset_dir):
            cls_dir = os.path.join(self.dataset_dir, cls)   # data/pointda/shapenet/bed
            
            if os.path.isdir(cls_dir):
                dir_f = os.path.join(cls_dir, split)    # data/pointda/shapenet/bed/train
                label = name2idx[cls]
