...
Key difference from Exp5: the binary top-1 update gate is replaced by a
soft per-class proportional gate. Every class contributes to prototype
update proportional to its softmax probability, with a relaxed matching
threshold for higher-confidence classes.
...
class Exp10SoftPatchGateAdapter(BaseAdapter):
    """Three-way fusion with soft per-class proportional patch update gate.

    Instead of a binary top-1 gate (only the highest-confidence class updates),
    every class with softmax probability > soft_gate_threshold contributes to
    the patch-level prototype update. Higher-confidence classes get a relaxed
    matching threshold (easier to match patches).
    """
    ...
    def __init__(self, cfg):
        ...
        # Soft gate threshold — minimum softmax prob to trigger per-class update
        self.soft_gate_threshold = float(self.cfg.get("soft_gate_threshold", 0.1))
    ...
    def run(
        ...
        if not hasattr(clip_model.visual, "positional_embedding"):
            raise ValueError(
                "Exp10SoftPatchGate requires a ViT backbone (ViT-B/16) for patch extraction. "
        ...
        variance_min       = float(self.cfg.get("variance_min", 0.001))
        variance_max       = float(self.cfg.get("variance_max", 1.0))
        soft_gate_threshold = self.soft_gate_threshold
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(
                tqdm(loader, desc=f"[Exp10] {dataset_name}")
            ):
                ...
                # 7) Online memory update (soft per-class proportional gate)
                pred_conf = F.softmax(clip_logits, dim=-1).squeeze(0)  # [C]
                for c in range(C):
                    weight = float(pred_conf[c].item())
                    if weight > soft_gate_threshold:
                        # Weighted update: relax match_threshold proportional to confidence
                        # Higher confidence → easier to match (small relaxation)
                        relaxed_thresh = match_thresh - weight * 0.1
                        relaxed_thresh = max(match_thresh - 0.1, relaxed_thresh)  # floor
                        states[c] = self._update_state(
                            states[c], patches_norm, feat_norm, relaxed_thresh, max_K,
                        )

                if i % 500 == 0:
                    running = sum(accuracies) / len(accuracies)
                    print(f"---- Exp10 {running:.2f}% ----")

        final_acc = sum(accuracies) / len(accuracies)
        print(f"---- Exp10 FINAL {final_acc:.2f}% ----")

        with open("outputs/result.txt", "a") as f:
            f.write(
                f"Exp10SoftPatchGate's performance on {dataset_name}: "
                f"Top1- {final_acc:.2f}.\n"
            )
...
def build(cfg: dict) -> Exp10SoftPatchGateAdapter:
    """Factory function — called by runner.py via dynamic import."""
    return Exp10SoftPatchGateAdapter(cfg)
