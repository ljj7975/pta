"""
Experiment 12: Patch Quality Modulation of Image-Level Prototype Update
=======================================================================

Extends Exp5's three-way fusion with a quality-gated modulation of the
...
Key difference from Exp5:
  ...
  When quality_gate is high (patch-level evidence is discriminative),
  the image-level prototype update is amplified proportionally to
  `quality_modulation`.  When quality_gate is low, the update proceeds
  at the default rate.

  This reorders Exp5's loop: patch extraction + quality gate computation
  now happens before the image-level prototype update, allowing quality
  information from patches to influence the image-level adaptation.

Same dual independent prototype systems as Exp5:
...
# ------------------------------------------------------------------
# Local variant of update_text_features with quality-gated modulation
# ------------------------------------------------------------------

def update_text_features_with_quality(
    ...
    """
    Local variant of update_text_features with quality-gated EMA modulation.
    ...
    All other logic is identical to models.pta.update_text_features.
...
class Exp12PatchQualityModulationAdapter(BaseAdapter):
    ...
    def __init__(self, cfg):
        ...
        # Quality modulation strength (0.0 disables the effect)
        self.quality_modulation = float(self.cfg.get("quality_modulation", 1.0))

    # ------------------------------------------------------------------
    # Patch-level prototype state (Gaussian, same as Exp3/Exp5)
    # ------------------------------------------------------------------
    ...
    def _update_state(
        ...
        """
        Same update logic as Exp5/Exp3.
    ...
    def run(
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(
                tqdm(loader, desc=f"[Exp12] {dataset_name}")
            ):
                ...
                # 2) Patch-level Gaussian prototype scores  ← MOVED UP
                #    (before image-level proto update so quality_gate
                ...
                # 3) Adaptive evidence weighting + quality gate  ← MOVED UP
                ...
                refine_feature, target_prototype = update_text_features_with_quality(
                    image_features,
                    soft_logits.half(),
                    refine_feature,
                    target_prototype,
                ...
                if i % 500 == 0:
                    ...
                    print(f"---- Exp12 {running:.2f}% ----")

        final_acc = sum(accuracies) / len(accuracies)
        print(f"---- Exp12 FINAL {final_acc:.2f}% ----")

        with open("outputs/result.txt", "a") as f:
            f.write(
                f"Exp12PatchQualityModulation's performance on {dataset_name}: "
                f"Top1- {final_acc:.2f}.\n"
            )
...
def build(cfg: dict) -> Exp12PatchQualityModulationAdapter:
    """Factory function — called by runner.py via dynamic import."""
    return Exp12PatchQualityModulationAdapter(cfg)
