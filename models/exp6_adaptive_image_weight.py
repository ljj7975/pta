...
When a class has many good prototypes (proto_alpha[c] ≈ alpha_max), its
image-level weight drops toward tau_image_min (default 50). When a class
has few prototypes (proto_alpha[c] ≈ 0), the weight stays at tau_image_max
(default 100).
...
Key difference from exp5:
  - Exp5 uses a single scalar tau_image_proto for ALL classes.
  - Exp6 makes tau_image per-class, inversely proportional to proto_alpha
    (evidence of how many prototypes exist for that class).
"""
import os
...
class Exp6AdaptiveImageWeightAdapter(BaseAdapter):
    """Three-way fusion with per-class adaptive image-level weighting.

    The image-level prototype weight (tau_image) is computed per class:
      tau_image[c] = tau_image_max - (tau_image_max - tau_image_min) * proto_alpha[c]

    This allows classes with rich prototype coverage to rely less on the
    image-level signal and more on the patch-level Gaussian prototypes.
    """
    ...
    def run(
        ...
        if not hasattr(clip_model.visual, "positional_embedding"):
            raise ValueError(
                "Exp6AdaptiveImageWeight requires a ViT backbone (ViT-B/16) "
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(
                tqdm(loader, desc=f"[Exp6] {dataset_name}")
            ):
                ...
                # 6) Per-class adaptive image-level weight
                #    tau_image[c] = tau_image_max - (tau_image_max - tau_image_min) * proto_alpha[c]
                #    When proto_alpha[c] ≈ 0 (no prototypes) → tau_image[c] ≈ tau_image_max (100)
                #    When proto_alpha[c] ≈ alpha_max (saturated) → tau_image[c] ≈ tau_image_min (50)
                tau_image_per_class = tau_image_max - (tau_image_max - tau_image_min) * proto_alpha  # [C]
                ...
                if i % 500 == 0:
                    ...
                    print(f"---- Exp6 {running:.2f}% ----")

        final_acc = sum(accuracies) / len(accuracies)
        print(f"---- Exp6 FINAL {final_acc:.2f}% ----")

        with open("outputs/result.txt", "a") as f:
            f.write(
                f"Exp6AdaptiveImageWeight's performance on {dataset_name}: "
                f"Top1- {final_acc:.2f}.\n"
            )
...
def build(cfg: dict) -> Exp6AdaptiveImageWeightAdapter:
    """Factory function — called by runner.py via dynamic import."""
    return Exp6AdaptiveImageWeightAdapter(cfg)
