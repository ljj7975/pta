...
Gaussian prototype — with **per-class adaptive** image-level weighting
using the **inverted** formula:

    final_logits = tau_text       * clip_logits
                 + tau_image[c]   * image_proto_logits[c]
                 + tau_patch_proto * proto_alpha[c] * quality_gate * raw_proto[c]

Where tau_image[c] dynamically adjusts per class based on prototype quality:

    tau_image[c] = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha[c]

This is the **inverse** of Exp6: when a class has many good prototypes
(proto_alpha[c] ≈ alpha_max), its image-level weight now rises toward
tau_image_max (default 100). When a class has few prototypes
(proto_alpha[c] ≈ 0), the weight stays at tau_image_min (default 50).

Same dual independent prototype systems as exp4/exp5/exp6:
...
Key difference from exp6:
  - Exp6 uses tau_image inversely proportional to proto_alpha
    (more prototypes → less weight on image level).
  - Exp7 inverts the relationship: more prototypes → MORE weight on image level,
    testing whether the image-level prototype should be amplified when
    patch-level evidence is abundant.
"""
import os
...
class Exp7InvertedImageWeightAdapter(BaseAdapter):
    """Three-way fusion with per-class adaptive image-level weighting (inverted).

    The image-level prototype weight (tau_image) is computed per class:
      tau_image[c] = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha[c]

    Unlike Exp6 (where more prototypes reduce image weight), Exp7 amplifies
    the image-level signal when a class has abundant prototype evidence.
    """
    ...
    def __init__(self, cfg):
        ...
        self.tau_image_max   = float(self.cfg.get("tau_image_max", 100.0))
        self.tau_image_min   = float(self.cfg.get("tau_image_min", 50.0))
        self.tau_patch_proto = float(self.cfg.get("tau_patch_proto", 20.0))

    # ------------------------------------------------------------------
    # Patch-level prototype state (Gaussian, same as Exp3/Exp4/Exp5)
    # ------------------------------------------------------------------
    ...
    def run(
        ...
        if not hasattr(clip_model.visual, "positional_embedding"):
            raise ValueError(
                "Exp7InvertedImageWeight requires a ViT backbone (ViT-B/16) "
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(
                tqdm(loader, desc=f"[Exp7] {dataset_name}")
            ):
                ...
                # 3) Image-level proto logits (no scaling here — applied per-class below)
                image_proto_logits = (
                ...
                # 5) Adaptive evidence weighting (per-class alpha from n_images)
                ...
                # 6) Per-class adaptive image-level weight (INVERTED vs Exp6)
                #    tau_image[c] = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha[c]
                #    When proto_alpha[c] ≈ 0 (no prototypes) → tau_image[c] ≈ tau_image_min (50)
                #    When proto_alpha[c] ≈ alpha_max (saturated) → tau_image[c] ≈ tau_image_max (100)
                tau_image_per_class = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha  # [C]

                # 7) Three-way fusion with per-class adaptive image weight
                final_logits = (
                ...
                if i % 500 == 0:
                    ...
                    print(f"---- Exp7 {running:.2f}% ----")

        final_acc = sum(accuracies) / len(accuracies)
        print(f"---- Exp7 FINAL {final_acc:.2f}% ----")

        with open("outputs/result.txt", "a") as f:
            f.write(
                f"Exp7InvertedImageWeight's performance on {dataset_name}: "
                f"Top1- {final_acc:.2f}.\n"
            )
...
def build(cfg: dict) -> Exp7InvertedImageWeightAdapter:
    """Factory function — called by runner.py via dynamic import."""
    return Exp7InvertedImageWeightAdapter(cfg)
