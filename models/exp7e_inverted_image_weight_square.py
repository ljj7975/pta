"""
Experiment 7e: Inverted Per-Class Image-Level Weighting — Square Mapping
=========================================================================

Same three-way fusion as Exp7, but with a **square mapping** for the per-class
adaptive image-level weight:

    tau_image[c] = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha[c]²

The square function grows slower than linear for small proto_alpha values,
suppressing image-level weight for low-evidence classes and only amplifying
it when prototype evidence is abundant. This tests whether being more
selective about which classes get the image-level boost improves results.
...
class Exp7eInvertedImageWeightSquareAdapter(BaseAdapter):
    """Three-way fusion with square-mapped per-class adaptive image-level weighting.

    The image-level prototype weight (tau_image) is computed per class:
      tau_image[c] = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha[c]²

    Compared to Exp7 (linear), square suppresses low-evidence classes and only
    amplifies classes with abundant prototype coverage.
    """
    ...
    def run(
        ...
        if not hasattr(clip_model.visual, "positional_embedding"):
            raise ValueError(
                "Exp7eInvertedImageWeightSquare requires a ViT backbone (ViT-B/16) "
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(
                tqdm(loader, desc=f"[Exp7e] {dataset_name}")
            ):
                ...
                # ── SQUARE mapping (key difference from Exp7) ────────────
                # tau_image[c] = tau_image_min + (tau_image_max - tau_image_min) * proto_alpha[c]²
                tau_image_per_class = tau_image_min + (tau_image_max - tau_image_min) * (proto_alpha ** 2)
                ...
                if i % 500 == 0:
                    ...
                    print(f"---- Exp7e {running:.2f}% ----")

        final_acc = sum(accuracies) / len(accuracies)
        print(f"---- Exp7e FINAL {final_acc:.2f}% ----")

        with open("outputs/result.txt", "a") as f:
            f.write(
                f"Exp7eInvertedImageWeightSquare's performance on {dataset_name}: "
                f"Top1- {final_acc:.2f}.\n"
            )
...
def build(cfg: dict) -> Exp7eInvertedImageWeightSquareAdapter:
    return Exp7eInvertedImageWeightSquareAdapter(cfg)
