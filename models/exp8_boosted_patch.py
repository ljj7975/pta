"""
Experiment 8: Boosted Patch Prototype (25x tau_patch_proto)
=============================================================

Identical to Exp5 (Tunable Fusion) but with tau_patch_proto default
increased from 20.0 to 500.0 — a 25× boost — to test the effect of
heavier patch-level Gaussian prototype contribution in the three-way
fusion:
...
All other logic (dual prototype systems, update gates, adaptive evidence
weighting) is unchanged from Exp5.
"""
import os
...
class Exp8BoostedPatchAdapter(BaseAdapter):
    """Three-way fusion with boosted patch prototype weight (default
    tau_patch_proto=500.0, 25× the Exp5 default)."""
    ...
    def __init__(self, cfg):
        ...
        self.tau_patch_proto = float(self.cfg.get("tau_patch_proto", 500.0))
    ...
    def run(
        ...
        if not hasattr(clip_model.visual, "positional_embedding"):
            raise ValueError(
                "Exp8BoostedPatch requires a ViT backbone (ViT-B/16) for patch extraction. "
        ...
        with torch.no_grad():
            for i, (images, target) in enumerate(
                tqdm(loader, desc=f"[Exp8] {dataset_name}")
            ):
                ...
                if i % 500 == 0:
                    ...
                    print(f"---- Exp8 {running:.2f}% ----")

        final_acc = sum(accuracies) / len(accuracies)
        print(f"---- Exp8 FINAL {final_acc:.2f}% ----")

        with open("outputs/result.txt", "a") as f:
            f.write(
                f"Exp8BoostedPatch's performance on {dataset_name}: "
                f"Top1- {final_acc:.2f}.\n"
            )
...
def build(cfg: dict) -> Exp8BoostedPatchAdapter:
    """Factory function — called by runner.py via dynamic import."""
    return Exp8BoostedPatchAdapter(cfg)
