...
class GaussianPatchLevel(BasePatchLevel):
    ...
    def compute_patch_logits(self, images, encoder, states,
        ...
        # ── Extract patch embeddings (original image) ────────────────────────
        patch_embs = encoder.get_patch_embeddings(images, exclude_pos=exclude_pos)  # [P, D]
        patches_norm_orig = _safe_normalize(patch_embs)  # [P, D]

        # ── Patch relevance filtering (on original patch pool only) ───────────
        ...
        if (filter_mode != "none"
            ...
            keep_mask, filter_scores_per_class = filter_patches_by_text_alignment(
                patches_norm_orig, target_class_idx,
        ...
        # ── Augment patch evidence (toggleable) ───────────────────────────────
        # Generate edge-map-like and grayscale-like views, extract their patch
        # embeddings, apply the SAME relevance mask from the original image,
        # and concatenate all filtered embeddings before gaussian scoring.
        augment_patch_logits = bool(self._cfg.get("augment_patch_logits", False))
        if augment_patch_logits:
            img = images.squeeze(0)
            aug_views = [_edge_map_like(img), _to_grayscale_like(img)]
            aug_batch = torch.stack(aug_views, dim=0)
            aug_embs = encoder.get_patch_embeddings(aug_batch, exclude_pos=exclude_pos)
            aug_norm = _safe_normalize(aug_embs.reshape(-1, aug_embs.shape[-1]))
            if keep_mask is not None:
                keep_mask_aug = keep_mask.repeat(2)
                patches_norm = torch.cat(
                    [patches_norm_orig[keep_mask], aug_norm[keep_mask_aug]], dim=0
                )
            else:
                patches_norm = torch.cat([patches_norm_orig, aug_norm], dim=0)
        else:
            if keep_mask is not None:
                patches_norm = patches_norm_orig[keep_mask]  # [P_filtered, D]
            else:
                patches_norm = patches_norm_orig  # [P, D]
