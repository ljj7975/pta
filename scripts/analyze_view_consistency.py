#!/usr/bin/env python3
"""Phase 3b analysis: multi-view-consistency trust-adaptive fusion sweep.
...
LABEL_RE = re.compile(r"^VCP-(?P<setting>.+)-s(?P<seed>\d+)$")
...
def load_records(records_dir, setting, dataset, seed):
    label = f"VCP-{setting}-{dataset}-s{seed}"
...
def main():
    ...
    ap.add_argument("--result", default="outputs/result_view_consistency.txt")
    ap.add_argument("--records", default="outputs/records_view_consistency")
    ap.add_argument("--out", default="outputs/view_consistency_report.md")
    args = ap.parse_args()
    ...
    lines = ["# Phase 3b: Multi-View-Consistency Trust-Adaptive Fusion — Results", ""]
