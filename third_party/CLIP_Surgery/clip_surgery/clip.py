...
def load(name: str, adapter: bool = False, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None):
    """Load a CLIP model
    ...
    if not jit:
        model = build_model(name, state_dict or model.state_dict(), adapter).to(device)
        if str(device) == "cpu":
            model.float()
